<template>
    <el-popover
        ...
        popper-class="robo-select-multi-complex-popper"
        @show="isInputActive = true"
        @hide="isInputActive = false"
    >
        <div
            slot="reference"
            class="robo-select-multi-complex-reference"
            :class="isInputActive ? 'is-active' : ''"
            :style="{width: width + 'px'}"
        >
            <span v-if="title" class="robo-select-multi-complex-title">{{ title }}</span>
            <div class="robo-select-multi-complex-input">
            ...
            <i class="robo-select-multi-complex-select-icon el-icon-arrow-up"></i>
        </div>
        <div class="robo-select-multi-complex-content">
            <template>
...
import {
    ...
    TypeCacheConfigItem,
    TypeOptionsKeys
} from '@/plugins/robo-option-cache/config';

@Component({
    components: {RoboSelectAll, RoboOverflowText, RoboSelectMultiCheckbox}
})
export default class RoboSelectMultiComplex extends Vue {
    @Prop({type: String, required: true}) title!: string;
    @Prop({type: Array, required: true}) cacheConfig!: TypeCacheConfigs;

    @Prop({type: Object, required: true}) value!: TypeCacheValue;

    @Prop({type: Number, default: 280}) width!: string;
    @Prop({type: String, default: '请选择'}) placeholder!: string;

    @Emit('update:value')
    emitUpdate(newVal: TypeCacheValue) {
        return newVal;
    }

    @Emit('change')
    emitChange(newVal: TypeCacheValue) {
        return newVal;
    }
    ...
    get cacheOptionsGroups(): TypeCacheConfigItem[] {
        return this.cacheConfig.map((v) => {
            if (typeof v === 'string') {
                this.$roboOptions.getOptions(v);
                return {
                    cacheKey: v,
                    options: this.$roboOptions.cache[v].options,
                    groupTitle: this.$roboOptions.cache[v].title,
                    value: this.value[v]
                };
            }
            let options = v.options;
            if (!Array.isArray(options)) {
                this.$roboOptions.getOptions(v.cacheKey);
                options = this.$roboOptions.cache[v.cacheKey].options;
            }
            return {
                cacheKey: v.cacheKey,
                options: options,
                groupTitle: v.groupTitle || this.$roboOptions.cache[v.cacheKey].title,
    ...
    set selectedValue(newVal: TypeCacheValue) {
    ...
    updateSelectedValue(newVal: TypeCacheValue, cacheKey: TypeOptionsKeys) {
        this.selectedValue = {
            ...this.value,
            [cacheKey]: newVal
        };
...
.robo-select-multi-complex {
    &-reference {
        ...
        &.is-active {
            ...
            .robo-select-multi-complex-select-icon {
                transform: rotateZ(0);
            }
        }
    }
